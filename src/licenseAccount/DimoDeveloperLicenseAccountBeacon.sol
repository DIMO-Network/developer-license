 ...
 */
contract DimoDeveloperLicenseAccountBeacon is IERC1271 {
    /// @custom:storage-location erc7201:DIMOdevLicense.storage.DevLicenseCore
    struct DimoDeveloperLicenseAccountBeaconStorage {
    ...
    // keccak256(abi.encode(uint256(keccak256("DIMOdevLicense.storage.DimoDeveloperLicenseAccountBeacon")) - 1)) & ~bytes32(uint256(0xff))
    ...
    function _getLicenseAccountStorage() internal pure returns (DimoDeveloperLicenseAccountBeaconStorage storage $) {
        assembly {
